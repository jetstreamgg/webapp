...
export const ChatBubble = ({
  ...
  return (
    <div
      ...
      className={`flex flex-col gap-3 @2xl/chat:items-start xl:gap-2 ${user === UserType.user ? '@2xl/chat:flex-row-reverse' : 'xl:@2xl/chat:gap-0'}`}
    >
      <HStack
        className={`items-center space-x-2 ${user === UserType.user ? 'xl:self-end xl:@2xl/chat:self-start' : '@2xl/chat:items-start'}`}
      >
        {user === UserType.bot ? (
          <img
        ...
        <Text
          variant="medium"
          className={`block leading-4 text-textSecondary ${user === UserType.user ? 'xl:hidden' : ''}`}
        >
          {user}
        </Text>
      </HStack>
      <VStack
        className={`gap-2 ${user === UserType.user ? 'xl:w-fit xl:self-end xl:rounded-2xl xl:rounded-tr-[2px] xl:bg-primary xl:py-3 xl:pl-5 xl:pr-7' : '@2xl/chat:ml-10'}`}
      >
        <Text
          variant="medium"
          className={`hidden leading-4 text-textSecondary ${user === UserType.user ? 'xl:block' : ''}`}
        >
          {user}
        </Text>
        {isLoading ? (
